...
use Froiden\JWTAuth\Exceptions\PayloadException;
...
class Payload implements ArrayAccess, Arrayable, Countable, Jsonable, JsonSerializable
{
     ...
     *
     * @var \Froiden\JWTAuth\Claims\Collection
     */
    private $claims;
     ...
     * @param  \Froiden\JWTAuth\Validators\PayloadValidator  $validator
     ...
     *
     * @param  string  $claim
     * @return \Froiden\JWTAuth\Claims\Claim
     */
     ...
     * @param  mixed  $value
     *
     * @throws \Froiden\JWTAuth\Exceptions\PayloadException
     */
    #[\ReturnTypeWillChange]
    ...
    public function __call($method, $parameters)
    {
        if (preg_match('/get(.+)\b/i', $method, $matches)) {
            foreach ($this->claims as $claim) {
                if (get_class($claim) === 'Froiden\\JWTAuth\\Claims\\'.$matches[1]) {
